...
visit_cmat_muladd(isel_context* ctx, nir_intrinsic_instr* instr)
{
   ...
   case GLSL_TYPE_INT4: {
      opcode = aco_opcode::v_wmma_i32_16x16x16_iu4;
      neg_lo[0] = type_a == GLSL_TYPE_INT4;
      neg_lo[1] = type_b == GLSL_TYPE_INT4;
      break;
   }
   ...
   if (get_replicated_constant(instr->src[2].ssa, acc_stride, &constant)) {
      ...
      if (!constC.isLiteral()) {
         C = constC;
      } else if (opcode != aco_opcode::v_wmma_i32_16x16x16_iu8 && opcode != aco_opcode::v_wmma_i32_16x16x16_iu4) {
