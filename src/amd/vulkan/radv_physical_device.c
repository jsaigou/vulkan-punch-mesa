                         ...
                         void (*array_size_cb)(struct __vk_outarray *base, struct matrix_prop *prop))
{
   ...
   /* INT4 cooperative matrix types. These are non-standard — VK_COMPONENT_TYPE_SINT4_KHR
    * and VK_COMPONENT_TYPE_UINT4_KHR don't exist in the Vulkan spec. Using custom values
    * in the EXT range until Khronos standardizes 4-bit types. */
#define VK_COMPONENT_TYPE_UINT4_CUSTOM ((VkComponentTypeKHR)1000672005)
#define VK_COMPONENT_TYPE_SINT4_CUSTOM ((VkComponentTypeKHR)1000672006)
   for (unsigned asigned = 0; asigned < 2; asigned++) {
      for (unsigned bsigned = 0; bsigned < 2; bsigned++) {
         for (unsigned csigned = 0; csigned < 2; csigned++) {
            for (unsigned saturate = 0; saturate < 2; saturate++) {
               ...
               prop.saturate = saturate;
               prop.a_type = asigned ? VK_COMPONENT_TYPE_SINT4_CUSTOM : VK_COMPONENT_TYPE_UINT4_CUSTOM;
               prop.b_type = bsigned ? VK_COMPONENT_TYPE_SINT4_CUSTOM : VK_COMPONENT_TYPE_UINT4_CUSTOM;
               prop.c_type = prop.r_type = csigned ? VK_COMPONENT_TYPE_SINT32_KHR : VK_COMPONENT_TYPE_UINT32_KHR;
